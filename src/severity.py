...
class SeverityClassifier:
    ...
    def classify_severity(self, area, granulation_pct, slough_pct, necrotic_pct):
        score = 0
        if area > LARGE_AREA_THRESHOLD:  score += 1
        if necrotic_pct > 0.20:          score += 1
        if slough_pct > 0.40:            score += 1
        if granulation_pct < 0.30:       score += 1

        if score >= 3:   return "severe", score
        elif score >= 2: return "moderate", score
        else:            return "mild", score